...
This workflow comes with a small set of data and config files for testing in the
`.test` directory.  This is a good place to go to see examples of the `config` files,
etc.  I print them out below here, too:

**.test/config/config.yaml**
```yaml
`r paste(readLines(".test/config/config.yaml"), collapse = "\n")`
```

**.test/config/bams.tsv**
```yaml
`r paste(readLines(".test/config/bams.tsv"), collapse = "\n")`
```

**.test/config/pwcomps.tsv**
```yaml
`r paste(readLines(".test/config/pwcomps.tsv"), collapse = "\n")`
```
